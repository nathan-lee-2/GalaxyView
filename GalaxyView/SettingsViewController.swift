...
class SettingsViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource{
    ...
    @IBAction func NightModeToggled(_ sender: UISwitch) {
        animateThemeChange()
    }
    
    func animateThemeChange() {
        if nightModeSwitch.isOn {
            UIView.animate(withDuration: 1, animations: {self.view.backgroundColor = UIColor(red:0.13, green:0.13, blue:0.13, alpha:1.0)})
            UIView.animate(withDuration: 1, animations: {self.nightModeLabel.textColor = UIColor.white})
            UIView.animate(withDuration: 1, animations: {self.notificationsLabel.textColor = UIColor.white})
            UIView.animate(withDuration: 1, animations: {self.yearsBetweenLabel.textColor = UIColor.white})
        }
        else{
            UIView.animate(withDuration: 1, animations: {self.view.backgroundColor = UIColor.white})
            UIView.animate(withDuration: 1, animations: {self.nightModeLabel.textColor = UIColor.black})
            UIView.animate(withDuration: 1, animations: {self.notificationsLabel.textColor = UIColor.black})
            UIView.animate(withDuration: 1, animations: {self.yearsBetweenLabel.textColor = UIColor.black})
        }
    }
